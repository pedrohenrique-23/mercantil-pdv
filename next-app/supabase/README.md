...
A migration `0004_cash_register_operations.sql` adiciona `open_cash_register`, `record_cash_movement` e `close_cash_register`. A abertura impede dois caixas abertos na mesma empresa e grava o saldo inicial no ledger. Entradas e saídas manuais são armazenadas com sinal positivo/negativo. O fechamento bloqueia o registro, soma o ledger, grava valor esperado, valor contado e diferença. As Server Actions correspondentes ficam em `next-app/lib/cash/actions.ts`; a UI em `cash-panel.tsx` oferece abertura, operação, conferência e histórico. A migration `0004` deve ser aplicada depois de `0001`, `0002` e `0003` no Supabase remoto.
